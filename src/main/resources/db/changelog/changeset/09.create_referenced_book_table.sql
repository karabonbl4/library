CREATE TABLE IF NOT EXISTS reference_book (
	...
	CONSTRAINT book_br_id_fk FOREIGN KEY (book_id) REFERENCES public.book(id) ON UPDATE CASCADE ON DELETE CASCADE,
	CONSTRAINT reference_br_id_fk FOREIGN KEY (reference_id) REFERENCES public.book(id)ON UPDATE CASCADE ON DELETE CASCADE
);

WITH subquery AS (SELECT ab.book_id, ab.author_id
				  FROM author_book ab
				  JOIN book b ON b.id = ab.book_id)
INSERT INTO reference_book (book_id, reference_id)
